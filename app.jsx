function App() {
  ...
  React.useEffect(() => {
    function keyHandler(e) {
      ...
      if (key === "backspace") {
        ...
        const found = words.find((w) => w.word === guess);
        ...
        const newColors = colors.map((r) => r.slice());
        for (let i = 0; i < 5; i++) {
        ...
        if (row === 5) {
          const tword = words.find((w) => w.word === target);
  ...
  }, [grid, col, row, words, validWords, target, colors, gameOver, invalid]);
        ...
        <div key={date}>
          ...
          <ul className="history-list">
            {history[date].map((g, idx) => (
              <li key={idx}>
                {idx + 1}. {g.result} - {g.attempts} intento{g.attempts > 1 ? "s" : ""}
              </li>
            ))}
          </ul>
        </div>
